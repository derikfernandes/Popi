...
import { prepareMermaidCode } from "../utils/sanitizeMermaid";
...
/** Renderiza Mermaid sob demanda (biblioteca carregada só ao abrir o fluxograma). */
export default function MermaidRenderer({ chartCode }: MermaidRendererProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [svgContent, setSvgContent] = useState<string>("");
  const [error, setError] = useState<string | null>(null);
  const [elementId] = useState(
    () => `mermaid-diagram-${++uniqueIdCount}-${Date.now()}`
  );
  ...
  useEffect(() => {
    ...
    const renderGraph = async () => {
      try {
        ...
        const cleanedCode = prepareMermaidCode(chartCode);
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Erro de renderização no compilador Mermaid. Sintaxe inválida.";
        setError(message);
        setSvgContent("");
      }
    };
        ...
        <div className="text-center p-6 space-y-3 max-w-xl animate-fade-in">
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-800">
              Sintaxe do Diagrama Inválida
            </h4>
            <p className="text-xs text-slate-500 mt-1">
              O modelo gerou um fluxograma que o compilador visual Mermaid não pôde
              processar.
            </p>
          </div>
          ...
          <p className="text-[11px] text-slate-400">
            Dica: Você pode editar o código do fluxograma manualmente para corrigir,
            ou usar um comando de edição por IA.
          </p>
        </div>
      ) : svgContent ? (
        <div
